...
CogDEI <- read.csv("/Users/edwinlam/Desktop/Zollman/CogDEI/cogg.csv",
                   skip = 6,
                   header = TRUE,
...
CogHomo <- read.csv("/Users/edwinlam/Desktop/Zollman/CogDEI/homo.csv",
                    skip = 6,
                    header = TRUE,
                    stringsAsFactors = FALSE,
                    check.names = FALSE) %>%
...
cog_models <- CogDEI %>%
  group_by(`num-turtles`, `network-structure`, `percent-egs-and-ri-agents`) %>%
  summarise(
    prob = mean(result, na.rm = TRUE),
    time = mean(`[step]`[result == 1], na.rm = TRUE),
    .groups = "drop"
  )
...
ggsave("coggg_prob.png", width = 8.5, height = 7, dpi = 300)